...
#include <unittests/nestdemo/o2nest.h>
#include "o2/src/o0globals.h"

//static const char *FbEndpoint = "https://graph.facebook.com/oauth/authorize?display=touch";
static const char *FbEndpoint = "https://home.nest.com/login/oauth2?client_id=eb613423-b658-4ab4-89d4-e29ab15d75bd&state=STATE";
static const char *FbTokenUrl = "https://graph.facebook.com/oauth/access_token";
static const char *FbExpiresKey = "expires";

O2Nest::O2Nest(QObject *parent): O2(parent) {
    setRequestUrl(FbEndpoint);
    setTokenUrl(FbTokenUrl);
}

void O2Nest::onVerificationReceived(const QMap<QString, QString> response) {
    qDebug() << "O2Nest::onVerificationReceived: Emitting closeBrowser()";
    Q_EMIT closeBrowser();

    if (response.contains("error")) {
        qWarning() << "O2Nest::onVerificationReceived: Verification failed";
        foreach (QString key, response.keys()) {
            qWarning() << "O2Nest::onVerificationReceived:" << key << response.value(key);
        }
...
void O2Nest::onTokenReplyFinished() {
    qDebug() << "O2Nest::onTokenReplyFinished";
    ...
    if (tokenReply->error() == QNetworkReply::NoError) {
        ...
        qWarning() << "O2Nest::onTokenReplyFinished:" << tokenReply->errorString();
    }
}
