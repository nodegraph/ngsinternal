...
void AppWorker::create_set_by_matching_text() {
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::create_set_by_matching_text_task,this));
}

void AppWorker::create_set_by_matching_images() {
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::create_set_by_matching_images_task,this));
}
...
void AppWorker::delete_set() {
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::delete_set_task,this));
}
...
void AppWorker::shift_to_text_above() {
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::shift_set_task<WrapType::text, Direction::up>,this));
}
void AppWorker::shift_to_text_below() {
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::shift_set_task<WrapType::text, Direction::down>,this));
}
void AppWorker::shift_to_text_on_left() {
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::shift_set_task<WrapType::text, Direction::left>,this));
}
void AppWorker::shift_to_text_on_right() {
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::shift_set_task<WrapType::text, Direction::right>,this));
}

void AppWorker::shift_to_images_above() {
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::shift_set_task<WrapType::image, Direction::up>,this));
}
void AppWorker::shift_to_images_below() {
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::shift_set_task<WrapType::image, Direction::down>,this));
}
void AppWorker::shift_to_images_on_left() {
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::shift_set_task<WrapType::image, Direction::left>,this));
}
void AppWorker::shift_to_images_on_right() {
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::shift_set_task<WrapType::image, Direction::right>,this));
}

void AppWorker::shift_to_inputs_above() {
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::shift_set_task<WrapType::input, Direction::up>,this));
}
void AppWorker::shift_to_inputs_below() {
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::shift_set_task<WrapType::input, Direction::down>,this));
}
void AppWorker::shift_to_inputs_on_left() {
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::shift_set_task<WrapType::input, Direction::left>,this));
}
void AppWorker::shift_to_inputs_on_right() {
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::shift_set_task<WrapType::input, Direction::right>,this));
}

void AppWorker::shift_to_selects_above() {
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::shift_set_task<WrapType::select, Direction::up>,this));
}
void AppWorker::shift_to_selects_below() {
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::shift_set_task<WrapType::select, Direction::down>,this));
}
void AppWorker::shift_to_selects_on_left() {
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::shift_set_task<WrapType::select, Direction::left>,this));
}
void AppWorker::shift_to_selects_on_right() {
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::shift_set_task<WrapType::select, Direction::right>,this));
}
...
void AppWorker::expand_above() {
  ...
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::expand_task,this, Direction::up, match_criteria));
}
...
void AppWorker::expand_below() {
  ...
  match_criteria[Message::kMatchFontSize] = true;
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::expand_task,this, Direction::down, match_criteria));
}
...
void AppWorker::expand_left() {
  ...
  match_criteria[Message::kMatchFontSize] = true;
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::expand_task,this, Direction::left, match_criteria));
}
...
void AppWorker::expand_right() {
  ...
  match_criteria[Message::kMatchFontSize] = true;
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::expand_task,this, Direction::right, match_criteria));
}
...
void AppWorker::mark_set() {
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::mark_set_task,this));
}

void AppWorker::unmark_set() {
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::unmark_set_task,this));
}
...
void AppWorker::shrink_set_to_topmost() {
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::shrink_set_to_side_task,this,Direction::up));
}
void AppWorker::shrink_set_to_bottommost() {
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::shrink_set_to_side_task,this,Direction::down));
}
void AppWorker::shrink_set_to_leftmost() {
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::shrink_set_to_side_task,this,Direction::left));
}
void AppWorker::shrink_set_to_rightmost() {
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::shrink_set_to_side_task,this,Direction::right));
}
...
void AppWorker::shrink_above_of_marked() {
  ...
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::shrink_against_marked_task,this,dirs));
}
...
void AppWorker::perform_click() {
  QVariantMap extra_args;

  unblock_events();
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::perform_action_task, this, ActionType::kSendClick, extra_args));
  block_events();
}
void AppWorker::perform_mouse_over() {
  QVariantMap extra_args;

  unblock_events();
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::perform_action_task,this, ActionType::kMouseOver, extra_args));
  block_events();
}
void AppWorker::type_text(const QString& text) {
  ...
  unblock_events();
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::perform_action_task,this,ActionType::kSendText,extra_args));
  block_events();
}
void AppWorker::type_enter() {
  ...
  unblock_events();
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::perform_action_task,this,ActionType::kSendEnter,extra_args));
  block_events();
}
void AppWorker::extract_text() {
  ...
  unblock_events();
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::perform_action_task,this,ActionType::kGetText,extra_args));
  block_events();
}
void AppWorker::select_from_dropdown(const QString& option_text) {
  ...
  unblock_events();
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::perform_action_task,this,ActionType::kSelectOption,extra_args));
  block_events();
}
...
void AppWorker::scroll_down() {
  ...
  unblock_events();
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::perform_action_task,this,ActionType::kScrollDown,extra_args));
  block_events();
}
void AppWorker::scroll_up() {
  ...
  unblock_events();
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::perform_action_task,this,ActionType::kScrollUp,extra_args));
  block_events();
}
void AppWorker::scroll_right() {
  ...
  unblock_events();
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::perform_action_task,this,ActionType::kScrollRight,extra_args));
  block_events();
}
void AppWorker::scroll_left() {
  ...
  unblock_events();
  queue_task(std::bind(&AppWorker::get_crosshair_info_task,this));
  queue_task(std::bind(&AppWorker::perform_action_task,this,ActionType::kScrollLeft,extra_args));
  block_events();
}
