...
int main(int argc, char *argv[]) {
  ...
  bootstrap_memory_tracker();
  {
    ...
    QApplication::setApplicationName("Robo Downloader"); // This affects the user's data dir resolution.
    QApplication::setOrganizationDomain("robodownloader.com");

    // Check to make sure that there are no other instance running.
    bool first_instance = true;

    QString tmpDir = QDir::tempPath();
    QLockFile lockFile(tmpDir + "/robodownloader.lock");
...
//      } else
      ...
      view->setSource(QUrl(QStringLiteral("qrc:/qml/robodownloader/fullpages/AlreadyRunningPage.qml")));
      view->rootObject()->setProperty("ngs_version", NGS_VERSION);
      view->show();
      view->update();
      // Run the Qt loop.
