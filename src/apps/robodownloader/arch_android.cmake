...
set(QML_ROOT_PATH ${CMAKE_SOURCE_DIR}/apps/robodownloader/qml)
set(APPLICATION_BINARY ${CMAKE_BINARY_DIR}/install_robodownloader/libs/armeabi-v7a/librobodownloader.so)
set(ANDROID_PACKAGE_SOURCE_DIRECTORY ${CMAKE_BINARY_DIR}/install_robodownloader_overrides)

# Make our main install_robodownloader output dir.
file(MAKE_DIRECTORY ${CMAKE_BINARY_DIR}/install_robodownloader/libs/armeabi-v7a)

# Make our install_robodownloader overrides dir.
file(MAKE_DIRECTORY ${CMAKE_BINARY_DIR}/install_robodownloader_overrides)
...
# ------------------------------------------------------------------------------------
# Custom command to prep the install_robodownloader dir.
# ------------------------------------------------------------------------------------
add_custom_command (
  DEPENDS install
  OUTPUT prep_robodownloader_cmd
  # copy the installed libs into the install_robodownloader/libs/armeabi-v7a folder
  COMMAND ${CMAKE_COMMAND} -E copy_directory ${CMAKE_BINARY_DIR}/install/lib ${CMAKE_BINARY_DIR}/install_robodownloader/libs/armeabi-v7a
  # copy the gdb server file over
  COMMAND ${CMAKE_COMMAND} -E copy ${ANDROID_NDK_ROOT}/prebuilt/android-arm/gdbserver/gdbserver ${CMAKE_BINARY_DIR}/install_robodownloader/libs/armeabi-v7a
  # rename gdbserver to gdbserver.so
  COMMAND ${CMAKE_COMMAND} -E rename ${CMAKE_BINARY_DIR}/install_robodownloader/libs/armeabi-v7a/gdbserver ${CMAKE_BINARY_DIR}/install_robodownloader/libs/armeabi-v7a/gdbserver.so
  COMMAND ${CMAKE_COMMAND} -E remove ${CMAKE_BINARY_DIR}/install_robodownloader/libs/armeabi-v7a/gdbserver
...
add_custom_target (prep_robodownloader
   DEPENDS prep_robodownloader_cmd
)
...
if (${CMAKE_BUILD_TYPE} STREQUAL "Debug")
    add_custom_command (
    DEPENDS prep_robodownloader
    OUTPUT install_robodownloader_cmd
    COMMAND ${ANDROID_DEPLOY_QT} 
  	    #--verbose
  	    --debug
  	    --output "${CMAKE_BINARY_DIR}/install_robodownloader"
...
if (${CMAKE_BUILD_TYPE} STREQUAL "Debug")
    ...
    COMMAND adb uninstall com.robodownloader.app
    COMMAND ${ANDROID_DEPLOY_QT} 
  	    #--verbose
  	    --debug
  	    --reinstall
