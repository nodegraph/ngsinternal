...
WebView {
    ...
    // The recording property determines whether the connect or disconnect is called on smashbrowse when pulsing the browser.
    ...
    function inject_smashbrowse(next_func) {
        var on_smashbrowse_loaded = app_settings.try_catch_wrap(function (result) {
            console.log("smashbrowse loaded")
            next_func()
        })

        var on_smashbrowse_checked = app_settings.try_catch_wrap(function (result) {
            //console.log("is smashbrowse installed:--" + result + "--")
            if (String(result) == "false") {
                console.log("smashbrowse not loaded .. trying to inject")
                web_view.runJavaScript(scripts.get_event_recorder_script(),on_smashbrowse_loaded)
            } else {
        ...
        var script = "  var result = true
                        if (typeof(smashbrowse) === \"undefined\") {
                            result = false
                        }
                        result"
        web_view.runJavaScript(script, on_smashbrowse_checked)
    }

    function connect_smashbrowse(next_func) {
...
//                    web_view.inject_smashbrowse(function() {
//                        web_view.connect_smashbrowse(function() {
