...
message("smashbrowse install: qt5_dir is: ${QT5_DIR}")
...
set(QML_ROOT_PATH ${CMAKE_SOURCE_DIR}/apps/smashbrowse/qml)
set(APPLICATION_BINARY ${CMAKE_BINARY_DIR}/install_smashbrowse/libs/armeabi-v7a/libsmashbrowse.so)
set(ANDROID_PACKAGE_SOURCE_DIRECTORY ${CMAKE_BINARY_DIR}/install_smashbrowse_overrides)

# Make our main install_smashbrowse output dir.
file(MAKE_DIRECTORY ${CMAKE_BINARY_DIR}/install_smashbrowse/libs/armeabi-v7a)

# Make our install_smashbrowse overrides dir.
file(MAKE_DIRECTORY ${CMAKE_BINARY_DIR}/install_smashbrowse_overrides)
...
# ------------------------------------------------------------------------------------
# Custom command to prep the install_smashbrowse dir.
# ------------------------------------------------------------------------------------
add_custom_command (
  DEPENDS install
  OUTPUT prep_smashbrowse_cmd
  # copy the installed libs into the install_smashbrowse/libs/armeabi-v7a folder
  COMMAND ${CMAKE_COMMAND} -E copy_directory ${CMAKE_BINARY_DIR}/install/lib ${CMAKE_BINARY_DIR}/install_smashbrowse/libs/armeabi-v7a
  # copy the gdb server file over
  COMMAND ${CMAKE_COMMAND} -E copy ${ANDROID_NDK_ROOT}/prebuilt/android-arm/gdbserver/gdbserver ${CMAKE_BINARY_DIR}/install_smashbrowse/libs/armeabi-v7a
  # rename gdbserver to gdbserver.so
  COMMAND ${CMAKE_COMMAND} -E rename ${CMAKE_BINARY_DIR}/install_smashbrowse/libs/armeabi-v7a/gdbserver ${CMAKE_BINARY_DIR}/install_smashbrowse/libs/armeabi-v7a/gdbserver.so
  COMMAND ${CMAKE_COMMAND} -E remove ${CMAKE_BINARY_DIR}/install_smashbrowse/libs/armeabi-v7a/gdbserver
...
add_custom_target (prep_smashbrowse
   DEPENDS prep_smashbrowse_cmd
)
...
if (${CMAKE_BUILD_TYPE} STREQUAL "Debug")
    add_custom_command (
    DEPENDS prep_smashbrowse
    OUTPUT install_smashbrowse_cmd
    COMMAND ${ANDROID_DEPLOY_QT} 
  	    #--verbose
  	    --debug
  	    --output "${CMAKE_BINARY_DIR}/install_smashbrowse"
...
if (${CMAKE_BUILD_TYPE} STREQUAL "Debug")
    ...
    COMMAND adb uninstall com.smashbrowse.app
    COMMAND ${ANDROID_DEPLOY_QT} 
  	    #--verbose
  	    --debug
  	    --reinstall
